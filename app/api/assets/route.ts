...
export async function GET() {
  ...
  if (!current)
    return NextResponse.json(
      { error: "Inicia sesión para ver tus medios." },
      { status: 401 },
    );
  ...
  return NextResponse.json({
    assets: (data ?? []).map((asset) => {
      const [resourceType = "image", format = "jpg"] =
        asset.mime_type.split("/");
      return {
        id: asset.id,
        publicId: asset.file_name,
        secureUrl: asset.storage_path,
        originalFilename: asset.file_name,
        resourceType,
        format,
        bytes: Number(asset.size_bytes),
...
export async function POST(request: Request) {
  try {
    ...
    if (!current)
      return NextResponse.json(
        { error: "Inicia sesión para guardar el medio." },
        { status: 401 },
      );
    ...
    if (error || !data)
      return NextResponse.json(
        { error: error?.message ?? "No se pudo registrar el medio." },
        { status: 500 },
      );
    return NextResponse.json(
      { asset: { ...asset, id: data.id, createdAt: data.created_at } },
      { status: 201 },
    );
  } catch {
    return NextResponse.json(
      { error: "Cloudinary devolvió un archivo no válido o mayor de 10 MB." },
      { status: 400 },
    );
  }
}
